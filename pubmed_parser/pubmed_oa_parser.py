...
def parse_pubmed_paragraph(path, all_paragraph=False):
    ...
    for sec in tree.xpath("//body/sec"):
        sec_name = sec.findtext("title")
        if sec_name is None:
            sec_name = ""
        sec_name = sec_name.strip().replace("/","")
    ...
    for paragraph in paragraphs:
        ...
        ### get the section information recursively to get the first-level section name and subsection names of a paragraph
        total_number_of_section_levels = 8
        section = []
        for i in range(total_number_of_section_levels):
            title_string = "../"*(i+1)+"title"
            sec_name = paragraph.findtext( title_string )
            if sec_name is None:
                continue
            sec_name = sec_name.strip().replace("/","")
            section.append(sec_name)
            if sec_name in section_names_set:
                break
        section.reverse()
